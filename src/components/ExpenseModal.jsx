import { useState } from 'react';
import { supabase } from '../lib/supabase';
import { useToast } from '../lib/ToastContext';

const CATEGORIES = [
  { id: 'purchasing', icon: '🛒', label: 'Purchasing' },
  { id: 'rent', icon: '🏠', label: 'Rent' },
  { id: 'utilities', icon: '💡', label: 'Utilities' },
  { id: 'salaries', icon: '👷', label: 'Salaries' },
  { id: 'maintenance', icon: '🔧', label: 'Maintenance' },
  { id: 'marketing', icon: '📣', label: 'Marketing' },
  { id: 'other', icon: '📦', label: 'Other' },
];

export default function ExpenseModal({ onClose, onSaved }) {
  const { showToast } = useToast();
  const [category, setCategory] = useState('purchasing');
  const [description, setDescription] = useState('');
  const [amount, setAmount] = useState('');
  const [expenseDate, setExpenseDate] = useState(new Date().toISOString().slice(0, 10));
  const [saving, setSaving] = useState(false);

  async function save() {
    if (!amount || Number(amount) <= 0) {
      showToast('Amount daalein', 'error');
      return;
    }
    setSaving(true);
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from('expenses').insert({
      category,
      description: description.trim() || null,
      amount: Number(amount),
      expense_date: expenseDate,
      recorded_by: user?.id || null,
    });
    setSaving(false);
    if (error) { showToast(error.message, 'error'); return; }
    showToast('Expense record ho gaya');
    onSaved();
  }

  return (
    <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl max-w-[360px] w-full shadow-2xl">
        <div className="bg-gradient-to-br from-maroon to-maroon2 p-5 rounded-t-2xl text-center">
          <div className="text-3xl mb-1.5">💸</div>
          <h2 className="text-white text-lg font-black">New Expense</h2>
        </div>
        <div className="p-5 space-y-3">
          <div>
            <label className="text-[11px] font-bold text-gray-500 uppercase">Category</label>
            <div className="grid grid-cols-4 gap-1.5 mt-1">
              {CATEGORIES.map((c) => (
                <button
                  key={c.id}
                  onClick={() => setCategory(c.id)}
                  className={`py-2 rounded-lg border-2 text-center transition-all ${
                    category === c.id ? 'border-orange bg-orange-50' : 'border-gray-100 hover:border-orange-200'
                  }`}
                >
                  <div className="text-base">{c.icon}</div>
                  <div className="text-[9px] font-bold mt-0.5">{c.label}</div>
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="text-[11px] font-bold text-gray-500 uppercase">Amount (Rs)</label>
            <input
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className="w-full mt-1 px-3 py-2 border-2 border-orange-200 rounded-lg outline-none focus:border-orange"
              placeholder="e.g. 3500"
              autoFocus
            />
          </div>
          <div>
            <label className="text-[11px] font-bold text-gray-500 uppercase">Description (optional)</label>
            <input
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full mt-1 px-3 py-2 border-2 border-orange-200 rounded-lg outline-none focus:border-orange"
              placeholder="e.g. Chicken purchase from Al-Karam"
            />
          </div>
          <div>
            <label className="text-[11px] font-bold text-gray-500 uppercase">Date</label>
            <input
              type="date"
              value={expenseDate}
              onChange={(e) => setExpenseDate(e.target.value)}
              className="w-full mt-1 px-3 py-2 border-2 border-orange-200 rounded-lg outline-none focus:border-orange"
            />
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 py-2.5 bg-gray-100 rounded-lg font-bold text-sm">Cancel</button>
            <button onClick={save} disabled={saving} className="flex-1 py-2.5 bg-orange text-white rounded-lg font-bold text-sm disabled:bg-gray-300">
              {saving ? 'Saving…' : 'Save Expense'}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
